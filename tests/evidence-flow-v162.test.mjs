...
test("current evidence preview and measured download improvements remain",()=>{
  assert.match(media,/evia-evidence-text-v191/);
  assert.match(media,/Media is loaded only after an item is opened/);
  assert.match(media,/function mediaType\(/);
  assert.match(media,/audioId/);
