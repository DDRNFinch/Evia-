...
function setGroup(card,on){
  ...
  if(!em)return;
  em.textContent=on?"✓":"";
  em.classList.toggle("evia-group-check-v204",!!on);
}
...
function legend(){
  ...
  title.parentElement?.querySelectorAll(":scope > .evia-coverage-key-v203,:scope > .evia-coverage-key-v202,:scope > .evia-coverage-key-v107").forEach(x=>x.remove());
  let key=title.parentElement?.querySelector(":scope > .evia-coverage-key-v204");
  if(key)return;
  key=document.createElement("div");
  key.className="evia-coverage-key-v204";
...
function hookStorage(){
  if(window.__eviaEvidenceStateStorageV204)return;
  window.__eviaEvidenceStateStorageV204=true;
...
style.id=STYLE_ID;
style.textContent=`
.selfobs .self-ksbs button[data-code]>span:not(.evia-ksb-marker-rail-v204){display:none!important}
...
window.EviaEvidenceStateV204=api;
