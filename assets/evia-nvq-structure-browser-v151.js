(()=>{
"use strict";
const VERSION=151,RPL_KEY="evia-rpl-ksbs-v1",EVIDENCE_KEY="evia-selfobs-live-v3",OBS_KEY="evia-mini-milos-observed-v1";
...
function row(id,title,st,attr,extra=""){return `<button type="button" class="evia-tools-row evia-acb-theme-row" ${attr}="${esc(id)}"><span><b>${esc(title)}</b><small>${st.covered}/${st.total} ACs covered${extra}</small></span><em>›</em></button>`}
function openGroups(){
...
function status(code,s){const r=s.rpl.has(code),n=s.evidence.get(code)||0,o=s.observed.has(code);if(!r&&!n&&!o)return'<span class="evia-acb-status empty">Not covered</span>';return `<span class="evia-acb-status">${r?'<i class="evia-acb-rpl" title="Recognised prior learning">o</i>':""}${n?`<i class="evia-acb-evidence" title="Evidence items">${n>5?`o × ${n}`:"o".repeat(n)}</i>`:""}${o?'<i class="evia-milos-arch-marker" title="Observed as competent by assessor in Milos">o</i>':""}</span>`}
