...
function splitCameraGallery(root=document){
  ...
  root.querySelectorAll?.('.evia-practical-upload').forEach(label=>{
    ...
    const camera=document.createElement("button");camera.type="button";camera.textContent="Camera";
    const gallery=document.createElement("button");gallery.type="button";gallery.textContent="Gallery";
...
function start(){
  apply();setTimeout(apply,100);setTimeout(apply,350);
