(()=>{
"use strict";
const VERSION=223,STORE="evia-selfobs-live-v3",PACK_KEY="nisi-installed-course-packs-v1",TIMELINE_KEY="evia-course-timeline";
let queued=false;
...
function state(){
 ...
 for(const code of a.codes){const key=String(code).toUpperCase(),parts=a.criteria[code]||a.criteria[key];if(Array.isArray(parts)&&parts.length){if(other.has(key)||parts.every(x=>byOpp.has(String(x.opportunityId))))covered.add(key)}else if(other.has(key)||byCode.has(key))covered.add(key)}
 return{...a,covered,entries,byOpp}
}
...
function patch(){
 ...
 document.querySelectorAll(".self-ksbs button[data-code]").forEach(btn=>{const code=String(btn.dataset.code||"").toUpperCase(),parts=s.criteria[code];if(Array.isArray(parts)&&parts.length&&!s.covered.has(code))removeLearner(btn)});
